...
| 招聘 | BOSS、拉勾 | LinkedIn |
| 专利 | 国知局 | USPTO、EPO |
| 论文与开源 | arXiv 官方 API、Robotics_Notebooks 已有管道 | 同左 |
...
一篇论文常同时透露运控路线、模型形态、仿真栈和采集方式四条轴，密度远超其他源。入库后台已支持（`ingest/paper.py`）：粘 arXiv 编号、abs 链接或标题加摘要，按 `vocab/keywords.yaml` 抽出轴候选，**每条带命中词和原文片段**。

关键词与词表分开放在 `vocab/keywords.yaml`：词表是契约（改取值要迁移），关键词只是探测启发式，可随时增删。

两条必须守住的规矩：

**轴候选不是结论。** 摘要里出现 MuJoCo 可能在相关工作里，不在本文方法里。所以界面强制把原文片段摆出来，候选只标 ◆ 不预选——预选等于替人做决定。

**⚠️ 只从作者串判归属，绝不从摘要正文判。** 摘要里的「deployment on a Unitree G1」说的是论文用了谁家硬件，不是论文由谁所写。混在一起会把高校论文记成本体厂商的技术信号。arXiv 元数据不含机构字段，所以**论文一律不自动填主体**，必须人工判定；若这篇只有高校参与，就不该入库——本库只收产业主体。

还要留意：**论文的轴信号弱于产品发布**。它说明团队研究过什么，不说明产品里跑什么。
