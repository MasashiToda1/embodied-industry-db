...
**非产业买方不进 registry，走 `counterparties` 纯文本。** 景区、工厂、医院、政府部门这类买方是长尾且无边界的，做成实体只会让 registry 塞满没有产业层、也不会有自身事件的空壳。它们只作溯源记录，不做可查询维度——**真正要查的不是「哪个买家」，而是「什么场景、多少钱」**，那两项由 `target_scene` 轴和 `amount` 承担。`role: customer` 只留给买方本身也是产业主体的情况，比如本体厂商采购灵巧手。

**`amount` 可留空，但填了必须带币种。** 招投标是少数能直接给出金额的源，跨国比价没有币种就没有意义。数字写不准就整个字段不填，不要填估算值。
...
入库后台已支持（`ingest/tender.py`）：粘公告正文或整页 HTML，抽出项目名称、编号、采购人、中标人、金额、公告日期，中标人按别名匹配到 registry 主体。**只给候选不自动绑定——认错主体比没认出来更糟。** 采购人按约定进 `counterparties` 纯文本。

⚠️ **金额是这条管道最容易出错的地方。** 中文公告里「元」和「万元」混用，还有「壹佰贰拾叁万肆仟元整」这种大写写法，差一个量级就是一万倍脏数据。所以单位必须显式命中才取值，识别不了一律留空，并且金额小于 1000 时告警提示核对单位。用例见 `tests/test_tender.py`。
