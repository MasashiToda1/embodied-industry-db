...
产业名录和「产品发布／融资」时间线已经有人做得很好（见下节），本库不重复那部分，收窄到三件它们没做的事：

**一，技术栈可对比、收敛可观测。** 受控词表把技术与商业选择压成固定轴，历史取值不删，于是同一条轴上所有主体的取值可以按时间铺开，分散度下降就是收敛。**现有同类库的产品字段是自由文本，做不到这件事**，而它恰好是「技术收敛时谁选对了」这个问题的答案所在。

**二，商业信号。** 招投标（谁在买、多少钱）、部署交付、公开定价、招聘信号、交付形态、目标场景。同类库明确把「部署交付、订单、招聘、生产进展」划在收录范围外，这一层是空的。
...
| [RealXiaoze/humanoid-motion-intelligence](https://github.com/RealXiaoze/humanoid-motion-intelligence)（CC BY-NC-SA 4.0） | 公司与产品主表、公开信号时间线、数据集主表、求职信息 | 当线索索引：顺着它标注的原始来源去抓一手页面、自存快照、按本库 schema 记录 |
...
- **不重建公司名录与产品发布／融资时间线**（humanoid-motion-intelligence 已有且在维护）
